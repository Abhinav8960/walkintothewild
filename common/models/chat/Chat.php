...
class Chat extends \yii\db\ActiveRecord implements \common\interfaces\NewStatusInterface
{
    ...
    public function rules()
    {
        return [
            ...
            [['user_id', 'lead_id', 'recipient_user_id', 'status', 'chat_type', 'last_message_at', 'is_seen', 'is_quote_accept', 'created_at', 'created_by', 'updated_at', 'updated_by', 'is_call_request', 'sender_id', 'is_lead_chat_open_for_user'], 'integer'],
    ...
    public function getDisplayLabelUser()
    {
        ...
        $userOperator = SafariOperator::find()->where(['user_id' => $user ? $user->id : ''])->one();
        if ($userOperator) {
            $userName .= ' | (Operator)';
        }
        return $userName ;
    ...
    public function getDisplayLabelRecipient()
    {
        ...
        $recipientOperator = SafariOperator::find()->where(['user_id' => $recipient ? $recipient->id : ''])->one();
