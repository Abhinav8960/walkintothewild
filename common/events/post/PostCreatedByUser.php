...
class PostCreatedByUser extends Event
{
    public $userId;
    public $email;
    public $name;
    ...
    public function getTemplates()
    {
        $arr = [
            'email' => [
                [
                    'subject' => $this->username . ' Created a post',
    ...
    {
        $this->master_notification_template = MasterNotificationTemplate::find()->where(['id' => MasterNotificationTemplate::NEW_POST, 'status' => 1])->limit(1)->one();
    ...
    {
        return $this->engine->render($this->master_notification_template->title, ['username'=>$this->username]);
    }
