...
class ArticleSearch extends Article
{
    ...
    {
        $query =  Article::find()->andWhere(['article.status' => [Article::USER_PUBLISHED]])->andWhere("is_schedule=0 OR DATE(publish_date_time)<='" . date('Y-m-d') . "'");
    ...
    public static function recentpost($slug = null)
    {
        if ($slug) {
            return Article::find()
                ...
                ->andWhere(['article.status' => Article::USER_PUBLISHED]) // Filters by active status
            ...
            return Article::find()
                ->andWhere(['article.status' => [Article::USER_PUBLISHED]])
