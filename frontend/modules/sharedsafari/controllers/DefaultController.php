...
class DefaultController extends FrontendBaseController
{
    ...
    public function actionUpdateFixedDeparture($slug)
    {
        ...
        $model->status = ShareSafari::STATUS_ACTIVE;
        $model->action_url = '/sharedsafari/default/update-fixed-departure?slug=' . $slug . '';
        $model->action_validate_url = '/sharedsafari/default/update-departure-validate?id=' . $shared_safari_departure_model->id . '';
            ...
            if ($model->load($this->request->post())) {
                if ($model->validate()) {
                    ...
                    if ($model->shared_safari_departure_model->save(false)) {
                        ...
                        \Yii::$app->session->setFlash('success', 'Data Updated Successfully');
                        return $this->redirect(\yii\helpers\Url::toRoute(['/sharedsafari/default/view', 'slug' => $shared_safari_departure_model->slug]));
                    }
                }
            }
        } else {
