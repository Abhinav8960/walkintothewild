<?php

use yii\helpers\Url;

$webasset = $this->assetManager->getBundle('\frontend\assets\FrontAppAsset');
$this->params['baseurl'] = $webasset->baseUrl;

$this->title = $blog->title;
$this->params['title'] = $this->title;
?>
<section class="profile-wrapper">
    <div class="container-lg mb-5">
        <?= $this->render('@frontend/modules/profile/views/default/tablist', ['blog' => 'active', 'user' => $user]) ?>
    </div>
</section>
<?php if (Yii::$app->user->identity) { ?>
    <section class=" margin_bottomfooter ">
        <div class="container-lg" id="profile_container">
            <div class="row mb-4 justify-content-center">
                <div class="col-lg-12 col-xl-12 pe-lg-0">
                    <div class="row justify-content-center gx-xl-5 gx-lg-2">
                        <div class="col-lg-12 col-xl-12 col-xxl-12 pe-lg-5">
                            <div class="card py-3 border-0 rounded-3">
                                <div class="card-body ">
                                    <div class="aritcla-details">
                                        <h1 class="articald-title pb-3"><?= $blog->title ?></h1>
                                        <div class="aritcal_bigimg pb-4">
                                            <img src="<?= isset($blog->banner_image) ? $blog->bannerimagepath : $this->params['baseurl'] . '/img/articalbig.png' ?>" alt="" class="w-100">
                                        </div>

                                        <p><?= $blog->description ?> </p>

                                    </div>
                                    <div class="tags-wrapper  my-5 d-flex justify-content-between flex-wrap align-items-center">
                                        <div class="author_wrapper">
                                            <ul class="artical-info ">
                                                <?php if ($topics = $blog->blogtopics) {
                                                    foreach ($topics as $topic) { ?>
                                                        <li class="tgas_btns"><svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" width="20" height="20" x="0" y="0" viewBox="0 0 32 32" style="enable-background:new 0 0 512 512" xml:space="preserve" class="hovered-paths me-1">
                                                                <g>
                                                                    <path d="M18.885 7.215h-8.6a1.48 1.48 0 0 1-1.48-1.48V3.48c0-.817.663-1.48 1.48-1.48h8.6c.817 0 1.48.663 1.48 1.48v2.255a1.48 1.48 0 0 1-1.48 1.48zM28.83 17.99c-.92-.92-2.42-.92-3.34 0l-1.092 1.098 3.342 3.341 1.09-1.09c.93-.929.93-2.419 0-3.349zM23.34 20.15l-5.83 5.82c-.12.13-.2.29-.23.46l-.44 2.59c-.1.57.4 1.06.97.97l2.59-.44c.17-.03.33-.11.45-.23l5.83-5.83z" fill="#f9d600" opacity="1" data-original="#000000" class="hovered-path"></path>
                                                                    <path d="M23.11 4.61h-1.245v1.125a2.984 2.984 0 0 1-2.98 2.98h-8.6a2.983 2.983 0 0 1-2.98-2.98V4.61H6.04c-1.97 0-3.57 1.6-3.57 3.57v18.25c0 1.97 1.6 3.57 3.57 3.57h9.457a2.309 2.309 0 0 1-.135-1.238l.439-2.583c.078-.458.288-.881.604-1.226l6.365-6.356 1.655-1.664a3.816 3.816 0 0 1 2.253-1.086V8.19a3.559 3.559 0 0 0-3.568-3.58zM13.028 19.443H8.689a.9.9 0 0 1 0-1.8h4.339a.9.9 0 0 1 0 1.8zm5.312-5.108H8.69a.9.9 0 0 1 0-1.8h9.65a.9.9 0 0 1 0 1.8z" fill="#f9d600" opacity="1" data-original="#000000" class="hovered-path"></path>
                                                                </g>
                                                            </svg><?= isset($topic->blogname) ? $topic->blogname->title : ''; ?></li>
                                                <?php }
                                                } ?>


                                            </ul>
                                        </div>
                                    </div>
                                    <div class="tags-wrapper  my-5 d-flex justify-content-between flex-wrap align-items-center">

                                        <div class="author_wrapper pt-2">
                                            <ul class="artical-info flex-wrap">
                                                <li><i class="fa-solid fa-user"></i><a href="<?= Url::toRoute(['/profile/default/index', 'user_handle' => $user->user_handle]) ?>"><?= isset($blog->user) ? $blog->user->name : '' ?></a></li>
                                                <li><img src="<?= $this->params['baseurl'] ?>/img/comments.png" alt=""><a href="#comment-wrapper-section"><?= $blog->getBlogcomments()->where(['parent_id' => null, 'status' => 1, 'is_deleted' => 0])->count() ?> Comments</a></li>
                                                <li><i class="fa-solid fa-calendar-days"></i><?= date('M d, Y', strtotime($blog->blog_date)) ?></li>

                                            </ul>
                                        </div>
                                    </div>
                                    <div class="comment-wrapper padding-artical" id="comment-wrapper-section">
                                        <?= $this->render('_comment', [
                                            'model' => $model,
                                            'blog' => $blog,
                                            'user_handle' => $user->user_handle,
                                            // 'replymodel' => $replymodel,
                                        ]) ?>
                                    </div>
                                </div>
                            </div>

                        </div>

                    </div>
                </div>


            </div>
        </div>
    </section>
<?php } else { ?>

    <div class="container-lg" id="profile_container">
        <div class="row justify-content-center">
            <div class="col-xxl-11 margin_bottomfooter">
                <div class="card position-relative" style="min-height: 350px;">
                    <div class="card-body">
                        <div class="withoutlogedin">
                            <h6 class="fs-6 fw-bold">Blogs</h6>
                        </div>

                        <div class="logininfo text-center">
                            <h6>Please log in to view the Blog <br>information.</h6>
                            <div class="viewAllreview">
                                <a href="/site/login?authclient=google&referrer=<?= Url::toRoute(['/profile/blog/index', 'user_handle' => $user->user_handle]) ?>">Login</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
<?php } ?>


<div class="modal fade" id="modalFlag" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header flageHeader">
                <h6 class="modal-title fs-5" id="exampleModalLabel">
                    Report Content
                    <br>
                    <p>Please report inappropriate members and/or content to help our Trust & Safety team keep our Community safe for everyone.</p>
                </h6>
                <button type="button" class="btn_close" data-bs-dismiss="modal" aria-label="Close"><img src="<?= $this->params['baseurl'] ?>/img/Share-Safari/flag.png" alt=""></button>
            </div>

            <div class="modal-body modal_form">
                <div id='modalContent'></div>
            </div>

        </div>
    </div>
</div>

<?php
$script = <<< JS
     var loc= window.location.href;
   
        $('html, body').animate({
                'scrollTop' : $("#profile_container").position().top - 180
        });
       
JS;
$this->registerJs($script);
?>