...
<div class="comment_hightfixed">
    <?php
    if ($blog_comments = $blog->getBlogcomments()->andWhere(['parent_id' => null, 'is_deleted' => 0])->joinWith('user')->andWhere(['user.status' => 10, 'blog_comment.status' => 1])->all()) {
        foreach ($blog_comments as $blog_comment) {
            $replies = $blog_comment->getReplies()->andWhere(['is_deleted' => 0])->joinWith('user')->andWhere(['user.status' => 10, 'blog_comment.status' => 1])->all();
    ?>
            <div class="comments-persons eee">
                <div class="postcomment d-flex gap-3">
                    <div class="avatar">
                        <a href="<?= Url::toRoute(['/profile/default/index', 'user_handle' => $blog_comment->user && $blog_comment->user->user_handle <> '' ? $blog_comment->user->user_handle : '']) ?>">
                            <img src="<?= isset($blog_comment->user) ? $blog_comment->user->profileimage : '' ?>" alt="">
                        </a>
                    </div>
                    <div class="text_com">
                        <div class="requestContact d-flex gap-2 align-items-center font-color">
                            <a href="<?= Url::toRoute(['/profile/default/index', 'user_handle' => $blog_comment->user && $blog_comment->user->user_handle <> '' ? $blog_comment->user->user_handle : '']) ?>">
                                <h6 class="nameavatr"><?= isset($blog_comment->user) ? $blog_comment->user->name : "" ?></h6>
                            </a>
                            <span class="comment-date"><?= date("F j, Y", $blog_comment->created_at) . ' at ' . date("h:i A", $blog_comment->created_at) ?></span>
                        </div>
                        <p><?= $blog_comment->comment ?></p>
                        <?php
                        if (Yii::$app->user->identity && Yii::$app->user->id != $blog_comment->user_id) { ?>
                            <button class="reply_btn" data-id="<?= $blog_comment->id ?>" data-target="reply-form-<?= $blog_comment->id ?>"> <i class="fa-solid fa-reply me-1"></i>Reply </button>
                        <?php }
                        ?>
                    </div>
                    <div class="objec-flgs pe-md-3 pe-2">
                        <?php if ($blog_comment->user) {
                            if (Yii::$app->user->identity && $blog_comment->user_id != Yii::$app->user->id) { ?>
                                <img src="<?= $this->params['baseurl'] ?>/img/Share-Safari/flag.png" alt="" class="flagBtn" value="<?= Url::toRoute(['/profile/blog/flag', 'slug' => $blog->slug, 'blog_comment_id' => $blog_comment->id, 'user_handle' => $user_handle]) ?>">
                        <?php }
                        } ?>
                    </div>
                </div>
                <div class="comment-reply px-3">
                    <?php if ($replies) { ?>
                        <h6 class="card-brown-heading pb-2 ms-lg-4 ms-2 pt-2 toggle-replies" data-target="comment-container-<?= $blog_comment->id ?>">View <?= count($replies) ?> replies</h6>
                        <div class="blog-comment-container" id="comment-container-<?= $blog_comment->id ?>" style="display: none;">
                                        ...
                                        <div class="font-color">
                                            ...
                                            <?php if ($reply->user) {
                                                if (Yii::$app->user->identity && $reply->user_id != Yii::$app->user->id) { ?>
                                                    <img src="<?= $this->params['baseurl'] ?>/img/Share-Safari/flag.png" alt="" class="flagBtn" value="<?= Url::toRoute(['/profile/blog/flag', 'slug' => $blog->slug, 'blog_comment_id' => $reply->id, 'user_handle' => $user_handle]) ?>">
                                            <?php }
                                            }
                                            ?>
                                        </div>
                    ...
                    <?php if (Yii::$app->user->id) {  ?>
                        <div class="reply-form ms-lg-4 ms-2" style="display: none;" id="reply-form-<?= $blog_comment->id ?>">
...
?>
    ...
    <?php ActiveForm::end(); ?>
<?php } else {
    echo '<p class="px-3 pt-2 text-center">Please <a href="/site/login?authclient=google&referrer=' . Url::toRoute(['/profile/blog/view', 'slug' => $blog->slug, 'user_handle' => $user_handle]) . '" class="sign_intext parkrevieBtn text-center">Sign in</a> to Comment</p>';
} ?>
...
$reply_url = Url::toRoute(['/profile/blog/reply', 'slug' => $blog->slug, 'user_handle' => $user_handle]);
