...
$this->params['breadcrumbs'][] =  ['label' => 'Blog', 'url' => '/cms/blog'];
    ...
    <div class="container-lg">
        <div class="row">
            <div class="status-container">
                <div class="status-item <?php echo $blog->status == 1 ? 'status-published' : 'status-unpublished'; ?>">
                    <span class="status-label">User Status:</span>
                    <?php echo $blog->status == 1 ? "Published" : "UnPublished"; ?>
                    <button class="btn_userblog" style="background:#F48270 !important;color:black !important;padding: 10px 16px !important;" value="<?= \yii\helpers\Url::toRoute(['/cms/blog/blogdelete?id=' . $blog->id . '']) ?>"><i class="fas fa-trash me-1"></i>Delete</button>
                </div>
                <!-- <div class="status-item <?php echo $blog->is_approved == 1 ? 'status-published' : 'status-unpublished'; ?>">
                    <span class="status-label">Main Portal Status:</span>
                    <?php echo $blog->is_approved == 1 ? "Published" : "UnPublished"; ?>
                    <button class="btn_mainportalblog" style="background:#F7BF39 !important;color:black !important;padding: 10px 16px !important;" value="<?= \yii\helpers\Url::toRoute(['/cms/blog/approval?id=' . $blog->id . '']) ?>"><i class="fas fa-edit me-1"></i>Change Status</button>
                </div> -->
            </div>
        </div>
        <h2 class="mt-2">
            ...
            <div class="col-lg-8 col-xl-8 col-xxl-8 pe-lg-5">
                <div class="aritcla-details">
                    <div class="aritcal_bigimg pb-4">
                        <img src="<?= isset($blog->banner_image) ? $blog->bannerimagepath : $this->params['baseurl'] . '/img/articalbig.png' ?>" alt="" class="w-100">
                    </div>
                            ...
                            <?php if ($topics = $blog->blogtopics) {
                                foreach ($topics as $topic) { ?>
                                    <li><svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" width="20" height="20" x="0" y="0" viewBox="0 0 32 32" style="enable-background:new 0 0 512 512" xml:space="preserve" class="hovered-paths">
                                        ...
                                        </svg><a href="#"><?= isset($topic->blogname->title) ? $topic->blogname->title : ''; ?></a></li>
                            <?php }
                            } ?>
                ...
                <div class="tags-wrapper  my-5 d-flex justify-content-between flex-wrap align-items-center">
                    ...
                    <div class="author_wrapper">
                        <ul class="artical-info ">
                            <li><i class="fa-solid fa-user"></i><a href="#"><?= isset($blog->user) ? $blog->user->name : '' ?></a></li>
                            <li><img src="<?= $this->params['baseurl'] ?>/img/comments.png" alt=""><a href=""><?= $blog->getBlogcomments()->where(['parent_id' => null, 'status' => 1, 'is_deleted' => 0])->count() ?> Comments</a></li>
                            <li><i class="fa-solid fa-calendar-days"></i><?= date('M d, Y', strtotime($blog->blog_date)) ?></li>
...
function organizefunction() {
	$('.btn_userblog').on('click', function () {
...
function mainportalfunction() {
	$('.btn_mainportalblog').on('click', function () {
        $('#organize-modal').modal('show')
		.find('#userstatusmodalContent')
		.load($(this).attr('value'));
	});
