...
<div class="row">
    ...
    <?php
    if ($model->shared_safari_banner_model->shared_safari_banner) { ?>
        <div class="col-md-5">
            <?= $form->field($model, 'file')->fileInput()->label('Image (JPEG / JPG / PNG / 250kb)') ?>
        </div>

        <div class="col-md-1">
            <?php echo '<img src="' . $model->shared_safari_banner_model->imagepath . '" width="75" height="75"></img>'; ?>
        </div>
    <?php } else { ?>
        <div class="col-md-6">
            <?= $form->field($model, 'file')->fileInput()->label('Image (JPEG / JPG / PNG / 250kb)') ?>
    ...
    <?php if ($model->shared_safari_banner_model->id) { ?>
        <div class="col-md-6">
