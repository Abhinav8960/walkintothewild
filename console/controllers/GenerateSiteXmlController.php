...
class GenerateSiteXmlController extends Controller
{
    ...
    protected function get_package_site_pages($backend_actual_url)
    {
        $base_url = Yii::$app->params['frontend_url'];

        $packages = Package::find()->select(['id', 'package_slug', 'updated_at', 'total_view'])->where(['status' => true])->asArray()->all();

        SitePages::updateAll(['status' => 0, 'updated_at' => date('Y-m-d H:i:s')], ['content_type' => 'package', 'status' => true]);
        if (count($packages) > 0) {
            $xml_content = "<?xml version='1.0' encoding='UTF-8'?>";
            $xml_content .= "<urlset xmlns='http://www.sitemaps.org/schemas/sitemap/0.9'>";
            ...
            foreach ($packages as $package) {
                $url = $base_url . 'package/' . $package['package_slug'];
