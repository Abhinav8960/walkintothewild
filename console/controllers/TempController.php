...
class TempController extends Controller
{
    ...
    public function actionTempCall()
    {
        $calls = CallLog::find()->where(['IS NOT', 'recording_url', null])->andWhere(['file_path' => null, 'call_status' => 3, 'status' => 1])->all();

        if ($calls) {
            foreach ($calls as $call) {
                $this->callRecordingUploadToS3($call->id, $call->recording_url);
            }
        }
